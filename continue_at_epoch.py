...
tf.flags.DEFINE_string("model_name", "gtn_rec", "The name of model")
...
tf.flags.DEFINE_integer("num_edges", 1, "Number of edges (default: 1)")
tf.flags.DEFINE_integer("gtn_in", 1, "GTN in channels (default: 1)")
tf.flags.DEFINE_integer("gtn_out", 1, "GTN out channels (default: 1)")

tf.flags.DEFINE_integer("matrix_type", 1, "The type of adjacency matrix (0=zero,1=real,default:1)")

# Training hyper-parameters
tf.flags.DEFINE_integer("continue_at_epoch", 15, "Number of epochs (default: 15)")
...
if config.train_mode:
    with tf.Session(config=gpu_config) as sess:
        ...
        net = models.Beacon(sess, config.emb_dim, config.rnn_unit, config.alpha, MAX_SEQ_LENGTH, item_probs, A,
                            config.top_k,
                            config.batch_size, config.rnn_cell_type, config.dropout_rate, config.seed,
                            config.learning_rate, A.shape[1], config.gtn_out)
        ...
        recent_dir = 'epoch_' + str(config.continue_at_epoch)
