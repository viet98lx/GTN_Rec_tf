...
class Beacon(Model):
    ...
    def __init__(self, sess, emb_dim, rnn_units, alpha, 
                 ...
                 batch_size, rnn_cell_type, rnn_dropout_rate, seed, learning_rate, gtn_in_channels, gtn_out_channels):
        ...
        with tf.variable_scope(self.scope):
            ...
            self.gtn_weight_1 = tf.get_variable(dtype=tf.float32, initializer=tf.constant(uniform_init_gtn), name="conv1_w")
            self.gtn_weight_2 = tf.get_variable(dtype=tf.float32, initializer=tf.constant(uniform_init_gtn), name="conv2_w")
